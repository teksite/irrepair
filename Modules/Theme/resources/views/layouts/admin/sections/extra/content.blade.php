...
@php
    $random=rand(10,1000);
    $data=old('extra.'.$key) ??  $instance?->getMeta($key)  ?? $data  ?? [];
@endphp
<div>
         ...
         <div class="mb-3 md:w-1/2">
          ...
          <div class="mb-3">
              ...
              <div class="grid md:grid-cols-2 gap-6 items-center">
                  <div>
                      <x-main::input.label :value="__('title')" for="url-{{$random}}-title" />
                      <x-main::input.text  class="block w-full mt-3" id="url-{{$random}}-title" name="extra[{{$key}}][url][title]" :value="old('extra.'.$key.'.url.title') ?? $data['url']['title'] ?? ''"/>
                  </div>
                  <div>
                      <x-main::input.label :value="__('address')" for="url-{{$random}}-address" />
                      <x-main::input.text dir="ltr" class="block w-full mt-3" id="url-{{$random}}-address" name="extra[{{$key}}][url][address]" :value="old('extra.'.$key.'.url') ?? $data['url']['address'] ?? ''"/>
                  </div>
              </div>
          </div>
      </fieldset>
    ...
    <x-main::input.error :messages="$errors->get('extra.'.$key.'.url.title')" class="mt-2"/>
    <x-main::input.error :messages="$errors->get('extra.'.$key.'.url.address')" class="mt-2"/>
</div>

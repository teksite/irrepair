...
@php
    $data = isset($instance) ? $instance->getMeta($key) : [];
     if(old('extra.'.$key) !== null){
         $old=old('extra.'.$key);
         $headings="'".implode("','",$old['titles'])."'" ;
         $items=$old['options'] ?? [];

     }elseif ($data){
            $headings = $data['titles'] ? "'".implode("','",$data['titles'])."'" : '';
            $items = $data['options'] ?? [];
        }else{
                            ...
                            <tr>
                                <template x-for="(column, columnIndex) in row" :key="columnIndex">
                                    <td class="px-6 py-4 text-center">
                                        <x-main::input.textarea type="text"
                                                                x-bind:name="`extra[{{$key}}][options][${rowIndex}][]`"
                                                                    class="w-full !p-1"

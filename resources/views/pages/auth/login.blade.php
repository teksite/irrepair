<x-main::auth-layout>
    @section('title',__('login to the account'))
    <div class="p-3">
        ...
        <div class="my-3 w-fit flex  items-center gap-6 justify-center bg-gray-50/50 px-3 py-2 rounded-xl mx-auto">
            <x-sso::social />
        </div>
        <form method="POST" action="{{ route('login') }}" class="formAction">
            ...
            <div class="mb-6">
                <div class="bg-white flex items-center border border-slate-200 rounded-lg mb-3">
                    ...
                    <x-main::input.text type="text" id="username" :value="old('email')" name="email" title="{{__('username')}} / {{__('email')}}" class="block w-full"
                                        placeholder="{{__('enter your username')}}" required="required"/>
                </div>
            ...
            <div class="mb-6">
                <div class="bg-white flex items-center border border-slate-200 rounded-lg mb-3">
                    <x-main::input.label for="password" title="{{__('password')}}"  value="<i class='tkicon fill-none stroke-gray-700' size='20' data-icon='lock-closed'></i>"
                                         class="flex flex-col justify-center items-center !mb-0 rounded  p-3"/>
                    <x-main::input.text type="password" id="password" name="password" title="{{__('password')}}" required="required" class="block w-full" placeholder="{{__('enter your password')}}"/>
                </div>
            ...
            <div class="mb-3 flex justify-start items-center gap-3">
                ...
                <x-main::input.label for="remember" :value="__('remember me')" class="!mb-0 !text-base !text-gray-200"/>
            </div>
            <div class="mb-3 flex items-center gap-1">
                <x-main::input.label class="min-w-fit !mb-0 !text-gray-200" :value="__('captcha code')" for="captcha-code"/>
                <x-captcha::load/>
            </div>
